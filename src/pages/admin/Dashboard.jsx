...
const Dashboard = () => {
  ...
  const handleUpdateUrl = async (id, currentUrl) => {
    const newUrl = window.prompt('请输入新的引流链接 (如需随机分流请填 random:你的分组码)', currentUrl);
    
    if (newUrl === null || newUrl === currentUrl) return; // 取消或没修改
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-gray-500">正在进入后台...</div>;
  }
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-md p-6">
        ...
        <div className="flex justify-between items-center mb-8 border-b pb-4">
          <h1 className="text-2xl font-bold text-gray-800">引流落地页 - 极简管理后台</h1>
          <button 
            ...
            <div key={link.id} className="border border-gray-200 p-4 rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-gray-50 transition-colors">
              <div className="mb-4 md:mb-0">
                <div className="font-bold text-lg text-gray-900">
                  {/* 兼容可能的多语言结构 */}
                  {link.title?.zh || link.title?.en || link.title || '未命名按钮'}
                </div>
